...
with col1 :
    if status =='LCD':
        ...
        lcd_acm=(1 / (1 + np.exp(-(0.021*illum + 1.255*text -5.6))))*100 #수용도 계산
        df=pd.DataFrame({
        ...
        led_acm=(1 / (1 + np.exp(-(0.0064*illum + 1.66*text -6.2))))*100 #수용도 계산
        df=pd.DataFrame({
            '휘도대비 (cd/m\u00b2)': [illum], '글꼴높이 (mm)': [text]})
        st.dataframe(df, hide_index=True, width=500)
    ...
    if status =='LCD':
        ...
        if (acm_slider==100) | (acm_slider==0) :
            ...
            if back_il==0:
                ...
                lcd_text=(5.6-math.log(100/acm_slider-1)-0.021*(0.009*(bright)**2-0.02*bright)*(glass/100)/back_il)/1.255
            if lcd_text<0:
                lcd_text=0
            elif lcd_text>100:
                lcd_text=100
        ...
        if (acm_slider==100) | (acm_slider==0) :
            ...
            if back_il==0:
                ...
                led_text=(6.2-math.log(100/acm_slider-1)-0.0064*bright*(glass/100)/back_il)/1.66
            if led_text<0:
                led_text=0
            elif led_text>100:
                led_text=100
